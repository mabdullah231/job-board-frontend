...
const Skills = () => {
  ...
  const fetchSkills = async () => {
    try {
      const response = await axios.get(
        `${Helpers.apiUrl}admin/skill/all`,
  ...
  const handleAddEditSkill = async () => {
    try {
      if (selectedSkill) {
        await axios.post(
          `${Helpers.apiUrl}admin/skill/manage`,
          { name: skillName, id: selectedSkill.id },
          Helpers.getAuthHeaders()
        );
  ...
  const handleDelete = async (id) => {
    try {
      await axios.delete(
        `${Helpers.apiUrl}admin/skill/delete/${id}`,
        Helpers.getAuthHeaders()
      );
        ...
        <>
          ...
          <nav>
            <ul className="pagination">
              {Array.from(
                { length: Math.ceil(skills.length / skillsPerPage) },
                (_, i) => (
                  <li
                    key={i}
                    className={`page-item ${
